...
#[cfg(test)] mod tests {
    ...
    fn airdop() {
        ...
        match client.request_airdrop(&keypair.pubkey(), 2_000_000_000u64) {
            Ok(s) => {
            println!("Success! Check out your TX here:"); println!("https://explorer.solana.com/tx/{}?cluster=devnet", s.to_string());
            },
    ...
    fn transfer_sol() {
        let keypair = read_keypair_file("dev-wallet.json").expect("Couldn't find wallet file");
        let to_pubkey = Pubkey::from_str("6X9xwDPUGD6azipUmNPiW3daf3zaAebDNqY6G7JpeQop").unwrap();
...
fn enroll() {
    ...
    let args = CompleteArgs {
        github: b"0xraidr".to_vec()
    };
    let blockhash = rpc_client.get_latest_blockhash().expect("Failed to get recent blockhash");
    let transaction = WbaPrereqProgram::complete(&[&signer.pubkey(), &prereq, &system_program::id()],
        &args,
        Some(&signer.pubkey()),
        &[&signer],
        blockhash
    ...
    println!("Success! Check out your TX here: https://explorer.solana.com/tx/{}/?custer=devnet", signature);
}
    }
